...
void UOldPalOrderWidget::NativeConstruct()
{
	...
	oldpalScript3.Emplace(FString::Printf(TEXT("단골이자 자네 아버지의 친구로서 테스트를 해보고 싶어서 그만"))); // 22
	...
	oldpalScript3.Emplace(FString::Printf(TEXT("명의의 반은 내앞으로 되어있네, 허허"))); // 26
	...
	oldpalScript3.Emplace(FString::Printf(TEXT("그땐 이 오래된 바를 자네 맘대로 해도 좋네"))); // 36
	...
	oldpalScript3.Emplace(FString::Printf(TEXT("조지가 생각하고 느꼈던 것들을 한번 되돌아 보는것도 도움이 될지 몰라"))); // 45
	...
	oldpalScript3.Emplace(FString::Printf(TEXT("어쨋든 오늘 이야기는 여기까지 하지"))); // 48
	...
	oldpalScript3.Emplace(FString::Printf(TEXT("젓지말고, 흔들어서 만들어주게나, 쉽지?"))); // 52
