...
void UOldPalOrderWidget::NativeConstruct()
{
	...
	oldpalScript1.Emplace(FString::Printf(TEXT("네그로니... 인건가"))); // 10
	oldpalScript1.Emplace(FString::Printf(TEXT("무슨 칵테일인지 알수가 없군"))); // 11
	//---------------------------------------------------------------------------
	...
	oldpalScript1.Emplace(FString::Printf(TEXT("기대 이상으로 괜찮았군"))); // 16
	oldpalScript1.Emplace(FString::Printf(TEXT("어떤 칵테일인지는 모르겠지만 잘마셨네"))); // 17
	//---------------------------------------------------------------------------
	oldpalScript1.Emplace(FString::Printf(TEXT("오늘은 한잔만 마시고 가보겠네"))); // 18
